...
const getConfig = () => {
  ...
  const nsProd = env('EPAGOS_NS_PROD', 'https://api.epagos.net/').trim();
  const nsSandbox = env('EPAGOS_NS_SANDBOX', 'https://sandbox.epagos.net/').trim();
  const nsExplicit = env('EPAGOS_NS', '').trim();
  const ns = (
    nsExplicit ||
    (epagosEnv === 'sandbox' && nsSandbox ? nsSandbox : nsProd)
  ).trim();

  const versionProd = env('EPAGOS_VERSION_PROD', '2.0').trim();
  const versionSandbox = env('EPAGOS_VERSION_SANDBOX', '2.1').trim();
  const versionExplicit = env('EPAGOS_VERSION', '').trim();
  const version = (
    versionExplicit ||
    (epagosEnv === 'sandbox' ? versionSandbox : versionProd)
  ).trim();

  const tokenVersionProd = env('EPAGOS_TOKEN_VERSION_PROD', '').trim();
  const tokenVersionSandbox = env('EPAGOS_TOKEN_VERSION_SANDBOX', '').trim();
  const tokenVersionExplicit = env('EPAGOS_TOKEN_VERSION', '').trim();
  const tokenVersion = (
    tokenVersionExplicit ||
    (epagosEnv === 'sandbox'
      ? tokenVersionSandbox
      : tokenVersionProd) ||
    version
  ...
  const pagoVersionProd = env('EPAGOS_PAGO_VERSION_PROD', '').trim();
  const pagoVersionSandbox = env('EPAGOS_PAGO_VERSION_SANDBOX', '').trim();
  const pagoVersionExplicit = env('EPAGOS_PAGO_VERSION', '').trim();
  const pagoVersion = (
    pagoVersionExplicit ||
    (epagosEnv === 'sandbox' ? pagoVersionSandbox : pagoVersionProd) ||
    version
  ).trim();

  const qrVersionProd = env('EPAGOS_QR_VERSION_PROD', '').trim();
  const qrVersionSandbox = env('EPAGOS_QR_VERSION_SANDBOX', '').trim();
  const qrVersionExplicit = env('EPAGOS_QR_VERSION', '').trim();
  const qrVersion = (
    qrVersionExplicit ||
    (epagosEnv === 'sandbox' ? qrVersionSandbox : qrVersionProd) ||
    version
  ).trim();
  ...
  const endpointProd = env(
    ...
    'https://api.epagos.net/wsdl/2.0/index.php'
  ).trim();
...
const soapActionFor = ({ method, endpoint }) => {
  ...
  if (cfg.soapActionMode === 'endpoint') {
    const base = normalizeEndpointUrl(endpoint);
    if (!base) return '';
    return `${base}/${method}`;
  }
...
const fetchSoap = async ({ method, envelope, timeoutMs, endpointOverride }) => {
  ...
  try {
    ...
    const endpointRaw = String(endpointOverride || defaultEndpoint || '').trim();
...
export const obtenerTokenEpagos = async ({ timeoutMs = 20000 } = {}) => {
  ...
  const inner = `
<version>${escapeXml(cfg.tokenVersion)}</version>
...
export const solicitudPagoEpagos = async ({
  ...
  const inner = `
<version>${escapeXml(cfg.pagoVersion)}</version>
...
export const generarQrVinculadoEpagos = async ({
  ...
  const inner = `
<version>${escapeXml(cfg.qrVersion)}</version>
<credenciales>${fillTemplate(credencialesPagoInner, vars)}</credenciales>
<operaciones_qr>
